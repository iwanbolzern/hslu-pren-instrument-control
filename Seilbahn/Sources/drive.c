...
void drive(void* pvParameter){
	...
	for(;;){
		if(!queue_isEmpty(driveQueue)){							//Sind Inhalte in der commandQueue werden diese gelesen
			...
			for(int i = 0; i < commandByte[1]; i++){						//Alle restlichen zum Kommande gehörenden Bytes werden in das commandByte Array abgelegt
		...
		//Interpretierung und Aufbereitung der Parameter für DriveDistance und DriveToEnd
			if(commandByte[2] != 3){
				unsigned int distance = (commandByte[3]+commandByte[4]);
				unsigned int speedFromIm = commandByte[5];
				unsigned int speed = calculateSpeed(speedFromIm);
				unsigned int ticksToDrive = 	calculateTicksToDrive(distance);
